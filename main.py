...
def train_and_predict_mul_epochs(video_paths:list, csv_paths:list, num_epochs:int):
    predictions_all = []  # Store predictions for all videos and epochs
    ...
    for video_path, csv_path in zip(video_paths, csv_paths):
        ...
        # Train the ESN for multiple epochs
        for epoch in trange(num_epochs, desc=f'Training {video_path}'):
...
def train_and_predict(video_path, video_path_2, csv_path):
    ...
    # Predict bounding box coordinates for the entire video
    predictions = esn.predict(frames_2)
...
# Example usage
#video_path = os.path.join(DATA_DIR, 'pedestrian_video/crosswalk.avi')
#video_path_2 = os.path.join(DATA_DIR, 'pedestrian_video/fourway.avi')
#csv_path = os.path.join(DATA_DIR, 'pedestrian_video/crosswalk.csv')
#csv_path_2 = os.path.join(DATA_DIR, 'pedestrian_video/fourway.csv')
#output_video_path = 'predicted_video.avi'
